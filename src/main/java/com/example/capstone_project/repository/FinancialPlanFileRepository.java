...
public interface FinancialPlanFileRepository extends JpaRepository<FinancialPlanFile, Long> {
    @Query(value = " SELECT files.id AS fileId, term.name AS termName, department.name AS departmentCode, RANK() OVER(PARTITION BY plan.id ORDER BY files.createdAt ASC) AS version FROM Term term " +
